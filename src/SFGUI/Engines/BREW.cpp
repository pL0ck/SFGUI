...
namespace sfg 
{
	namespace eng 
	{
		...
		void BREW::ResetProperties() 
		{
			...
			SetProperty("ListBox", "HighlightedColor", sf::Color(0x5e, 0x5e, 0x5e));
			SetProperty("ListBox", "SelectedColor", sfg::ColorEx::AliceBlue);
		...
		std::unique_ptr<RenderQueue> BREW::CreateBorder( const sf::FloatRect& rect, float border_width, const sf::Color& light_color, const sf::Color& dark_color ) 
		{
			...
			// Top
			queue->Add(Renderer::Get().CreateLine(sf::Vector2f( rect.left, rect.top ),sf::Vector2f( rect.left + rect.width, rect.top ),	light_color,border_width));

			// Right
			queue->Add(Renderer::Get().CreateLine(sf::Vector2f( rect.left + rect.width, rect.top ),sf::Vector2f( rect.left + rect.width, rect.top + rect.height ),dark_color,border_width));

			// Bottom
			queue->Add(Renderer::Get().CreateLine(sf::Vector2f( rect.left + rect.width, rect.top + rect.height ),sf::Vector2f( rect.left, rect.top + rect.height ),dark_color,border_width));

			// Left
			queue->Add(Renderer::Get().CreateLine(sf::Vector2f( rect.left, rect.top + rect.height ),sf::Vector2f( rect.left, rect.top ),light_color,border_width));
		...
		std::unique_ptr<RenderQueue> BREW::CreateSlider( const sf::FloatRect& rect, sf::Color& background_color, float border_width, const sf::Color& border_color, int border_color_shift ) 
		{
			...
			queue->Add(	Renderer::Get().CreatePane(sf::Vector2f( left, top),sf::Vector2f( width, height ),border_width,background_color,border_color,border_color_shift));
