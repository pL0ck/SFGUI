 ...
 */
class SFGUI_API LayoutPanel : public Container {
	public:
		typedef std::shared_ptr<LayoutPanel> Ptr; //!< Shared pointer.
		typedef std::shared_ptr<const LayoutPanel> PtrConst; //!< Shared pointer.
		 ...
		 * Alias to AddEnd(...) for backward compatibility.
		 * @param widget Widget.
		 * @param expand Expand widget to highest possible size.
		 * @param fill Fill calculated size.
		 */
		void AddControl( Widget::Ptr widget, bool expand = true, bool fill = true );
		 ...
		 * @param fill Fill calculated size.
		 */
		void AddStart( Widget::Ptr widget, bool expand = true, bool fill = true );
		 ...
		 * @param fill Fill calculated size.
		 */
		void AddEnd( Widget::Ptr widget, bool expand = true, bool fill = true );
	...
	private:
		...
		LayoutPanel( Orientation orientation = Orientation::HORIZONTAL, float spacing = 0.f );
		...
		ChildrenCont m_LayoutPanel_children;
		float m_spacing;
